...
public class TpaCommand { //todo more successes and errors
    ...
    public static int request(CommandSourceStack source, ServerPlayer otherPlayer) throws CommandSyntaxException {
        if (source.getEntityOrException() instanceof ServerPlayer sender) {
            source.sendSuccess(() -> Component.translatable("commands.ozone_utilities.tpa.request", otherPlayer.getDisplayName()), false);
    ...
    public static int here(CommandSourceStack source, ServerPlayer otherPlayer) throws CommandSyntaxException {
        if (source.getEntityOrException() instanceof ServerPlayer sender) {
//            source.sendSuccess(() -> Component.translatable("commands.ozone_utilities.tpa.here", otherPlayer.getDisplayName()), true);
    ...
    public static int accept(CommandSourceStack source) throws CommandSyntaxException { //todo make sure to send messages to players on both sides
...
//                    source.sendSuccess(() -> Component.translatable("commands.ozone_utilities.teleport.cancel", otherPlayer.getDisplayName()), true);
    ...
    public static int cancel(CommandSourceStack source) throws CommandSyntaxException { //todo potentially a cleaner way to do this.
        if (source.getEntityOrException() instanceof ServerPlayer sender) {
            ...
            if (otherUUID.isPresent()) {
                ServerPlayer otherPlayer = source.getServer().getPlayerList().getPlayer(otherUUID.get());
                if (otherPlayer != null) {
                    source.sendSuccess(() -> Component.translatable("commands.ozone_utilities.tpa.cancel", otherPlayer.getDisplayName()), false);
                    otherPlayer.getData(OzoneDataAttachments.PLAYER).clearOtherPlayerRequestingTeleport();
                }
    ...
    private static int teleportToEntity(CommandSourceStack stack, ServerPlayer target, ServerPlayer entity) {
        ...
        stack.sendSuccess(() -> Component.translatable("commands.teleport.success.entity.single", target.getDisplayName(), entity.getDisplayName()), false);
        return 1;
    }
}
